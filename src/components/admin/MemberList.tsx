...
export default function MemberList({
  ...
  // 會員分頁（2026-08-06 Glo 要求）：每 50 位一頁；搜尋時返第 1 頁，名單縮細時夾返頁數
  ...
  const pagedMembers = useMemo(
    () =>
      members.slice((memberPage - 1) * MEMBERS_PER_PAGE, memberPage * MEMBERS_PER_PAGE),
    [members, memberPage],
  );
          ...
          {memberPageCount > 1 && (
            <div className="mt-4 flex items-center justify-center gap-3 text-[13px] text-txt-2">
              ...
              <span className="font-mono">
                第 {memberPage} / {memberPageCount} 頁 · 共 {members.length} 位
              </span>
              <button
                type="button"
